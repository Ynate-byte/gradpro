    ...
    <div className="flex gap-3">
      ...
      <div className="flex-1">
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground px-3 mt-1">
          ...
          <button
            type="button" // [FIXED]: Thêm type="button" để ngăn submit form cha
...
export function TaskComments({ comments = [], onAddComment, isSubmitting }) {
  ...
  const handleSubmit = () => {
    ...
    onAddComment(commentContent.trim(), replyingTo?.ID_BINHLUAN || null);
    setCommentContent('');
    setReplyingTo(null);
  };
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-4">
        {comments.map((comment) => (
          <CommentItem key={comment.ID_BINHLUAN} comment={comment} onReplyClick={handleReplyClick} />
        ))}
      </div>
        ...
        <div className="flex-1 space-y-2">
          {replyingTo && (
            <Badge variant="secondary">
              ...
              <button type="button" className="ml-2 font-bold" onClick={() => setReplyingTo(null)}>
                X
              </button>
            </Badge>
          )}
