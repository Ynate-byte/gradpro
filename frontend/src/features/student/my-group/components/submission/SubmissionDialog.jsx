...
import {
  Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle, DialogClose, DialogFooter // <--- [ĐÃ SỬA LỖI] Thêm DialogFooter
...
// Component chính: Dialog Nộp sản phẩm
export function SubmissionDialog({ isOpen, setIsOpen, phancong, planId }) {
  ...
  const {
    ...
    enabled: !!phancongId && isOpen, // Chỉ fetch khi dialog mở
    onError: () => {
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: (res) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['myGroupDetails', planId] }); // Cập nhật lại trạng thái nhóm
    },
    onError: (error) => {
  ...
  if (!phancong) return null; // An toàn nếu chưa có phân công
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="sm:max-w-3xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            {canSubmit 
              ? "Tải lên các file và đường link sản phẩm của nhóm."
              : "Bạn có một lần nộp đang chờ xác nhận hoặc đã hoàn thành."}
          </DialogDescription>
        </DialogHeader>

        {/* Phần Form (chỉ hiển thị khi được phép) */}
        {canSubmit && (
