...
const QuotaManager = () => {
  ...
  const handleAdjustQuota = async (departmentId, newQuota) => {
                      ...
                      <TableCell className="text-center font-bold text-primary">
                        <div className="flex items-center justify-center gap-2">
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleAdjustQuota(dept.ID_KHOA_BOMON, (dept.quota_assigned || 0) - 1)}
                            disabled={(dept.quota_assigned || 0) <= 0 || isSubmitting}
                            className="h-6 w-6 p-0"
                          >
                            -
                          </Button>
                          <span className="min-w-[2rem] text-center">{dept.quota_assigned || 0}</span>
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleAdjustQuota(dept.ID_KHOA_BOMON, (dept.quota_assigned || 0) + 1)}
