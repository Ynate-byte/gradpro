...
const planSchema = z.object({
    ...
    mocThoigians: z.array(
        ...
        }).refine(data => !data.NGAY_BATDAU || !data.NGAY_KETTHUC || data.NGAY_KETTHUC >= data.NGAY_BATDAU, {
            message: "Ngày kết thúc của mục phải sau hoặc bằng ngày bắt đầu.",
            path: ["NGAY_KETTHUC"],
        })
    ).min(1, "Phải có ít nhất một mốc thời gian."),
}).refine(data => !data.NGAY_BATDAU || !data.NGAY_KETHUC || data.NGAY_KETHUC >= data.NGAY_BATDAU, {
    message: "Ngày kết thúc phải sau hoặc bằng ngày bắt đầu.",
    path: ["NGAY_KETHUC"],
});

// *** MODIFICATION 1: MilestoneItem ***
...
const MilestoneItem = React.forwardRef(({ index, field, remove, form, handleProps, onDateBlur, style, ...props }, ref) => {
    return (
        ...
        <div ref={ref} style={style} {...props} className="grid grid-cols-12 gap-x-4 gap-y-2 items-start p-4 border rounded-lg bg-background hover:shadow-md relative group">
            ...
            <div className="col-span-12"><FormField name={`mocThoigians.${index}.TEN_SUKIEN`} control={form.control} render={({ field: fld }) => ( <FormItem><FormLabel>Nội dung (Mục {index + 1})*</FormLabel><FormControl><Input {...fld} /></FormControl><FormMessage /></FormItem> )}/></div>
            <div className="col-span-12"><FormField name={`mocThoigians.${index}.MOTA`} control={form.control} render={({ field: fld }) => ( <FormItem><FormLabel>Mô tả</FormLabel><FormControl><Textarea {...fld} className="min-h-[60px] resize-y" /></FormControl><FormMessage /></FormItem> )}/></div>
            <div className="col-span-6"><FormField name={`mocThoigians.${index}.NGAY_BATDAU`} control={form.control} render={({ field: fld }) => ( <FormItem><FormLabel>Bắt đầu*</FormLabel><FormControl><Input type="datetime-local" {...fld} onBlur={onDateBlur} /></FormControl><FormMessage /></FormItem> )}/></div>
            <div className="col-span-6"><FormField name={`mocThoigians.${index}.NGAY_KETTHUC`} control={form.control} render={({ field: fld }) => ( <FormItem><FormLabel>Kết thúc*</FormLabel><FormControl><Input type="datetime-local" {...fld} onBlur={onDateBlur} /></FormControl><FormMessage /></FormItem> )}/></div>
...
const SortableItemWrapper = ({ id, children }) => {
    ...
    return React.cloneElement(React.Children.only(children), {
        ...
        handleProps: listeners // Pass listeners (which MilestoneItem expects as 'handleProps')
    });
};
// No displayName needed

function adjustDateForWeekend(date) { /* (no changes) */
...
export default function PlanFormPage() {
    ...
    const handleStartDateChange = useCallback((selectedDateStr) => {
            ...
            if (!userModifiedEndDate) {
             ...
             (templateData.mau_moc_thoigians || []).forEach((milestone, index) => {
                if (!userModifiedMilestoneDates[index]) {
                    ...
                    form.setValue(`mocThoigians.${index}.NGAY_BATDAU`, format(eventStartDate, dateFormat), { shouldValidate: true });
                    form.setValue(`mocThoigians.${index}.NGAY_KETTHUC`, format(eventEndDate, dateFormat), { shouldValidate: true });
                }
            });
        } catch (e) {
            ...
            try {
                ...
                if (isEditMode) {
                    ...
                    fetchedMocThoigians = (template.mau_moc_thoigians || []).map(m => ({
                        id: crypto.randomUUID(), TEN_SUKIEN: m.TEN_SUKIEN, MOTA: m.MOTA || '',
                        NGAY_BATDAU: '', NGAY_KETTHUC: '',
                        _templateOffset: m.OFFSET_BATDAU, _templateDuration: m.THOI_LUONG,
                    }));
                    dataToLoad = {
                        HEDAOTAO: template.HEDAOTAO_MACDINH,
                        TEN_DOT: '', NAMHOC: '', HOCKY: '', KHOAHOC: '', NGAY_BATDAU: '', NGAY_KETHUC: '',
                ...
                form.reset({
                    ...
                    mocThoigians: (fetchedMocThoigians).map(m => ({
                        ...m,
                        id: isEditMode ? m.ID : m.id, // Use real ID for dnd-kit if editing
    ...
    const handlePreview = async () => {
            ...
            const firstErrorField = Object.keys(form.formState.errors)[0];
             if (firstErrorField) {
                 try {
                   ...
                   } catch (e) { console.warn("Could not focus on field:", firstErrorField); }
             }
            return;
        }
        setIsSubmitting(true);
        try {
            const formData = form.getValues();
             const payload = { ...formData, mocThoigians: formData.mocThoigians.map(m => ({ ...m, id: typeof m.id === 'number' ? m.id : null })) };
    ...
    if (isLoading) { return ( <div className="space-y-6 p-4 md:p-8"><Skeleton className="h-10 w-32" /><Skeleton className="h-10 w-64" /></div> ); }
        ...
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 p-4 md:p-8">
                <div className="flex items-center justify-between gap-4">
                    <div>
                        <Button type="button" variant="outline" size="sm" onClick={() => navigate('/admin/thesis-plans')}><ChevronLeft className="mr-2 h-4 w-4" /> Quay lại</Button>
                    ...
                    <div className="flex gap-2">
                        <Button type="button" variant="secondary" onClick={handlePreview} disabled={isSubmitting}>{isSubmitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Eye className="mr-2 h-4 w-4" />} Xem trước</Button>
                        <Button type="submit" disabled={isSubmitting}>{isSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} {isEditMode ? 'Lưu thay đổi' : 'Tạo kế hoạch'}</Button>
                    </div>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                    <Card className="lg:col-span-1 sticky top-[calc(theme(spacing.14)_+_theme(spacing.8))]"><CardHeader><CardTitle>Thông tin chung</CardTitle></CardHeader>
                        <CardContent className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <FormField name="NGAY_KETHUC" control={form.control} render={({ field }) => (<FormItem><FormLabel>Ngày kết thúc*</FormLabel><FormControl><Input type="date" {...field} onBlur={() => setUserModifiedEndDate(true)} /></FormControl><FormMessage /></FormItem>)}/>
                            </div>
                        </CardContent>
                    </Card>
                    <div className="lg:col-span-2 space-y-4">
                        <Card><CardHeader className="flex flex-row items-center justify-between"><CardTitle>Mốc thời gian ({fields.length})</CardTitle><Button type="button" variant="outline" size="sm" onClick={() => append({ id: crypto.randomUUID(), TEN_SUKIEN: '', NGAY_BATDAU: '', NGAY_KETTHUC: '', MOTA: '' })}><PlusCircle className="mr-2 h-4 w-4" /> Thêm mục</Button></CardHeader>
                            <CardContent>
                                    ...
                                    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                                        <SortableContext items={fields.map(field => field.id)} strategy={verticalListSortingStrategy}>
                                            <div className="space-y-4">{fields.map((field, index) => (
                                                <div key={field.id} className="group relative"> {/* Use field.id as key now */}
                                                    <SortableItemWrapper id={field.id}>
                                                        <MilestoneItem
                                                            ...
                                                            remove={remove}
                                                            form={form}
                                                            onDateBlur={() => setUserModifiedMilestoneDates(prev => ({ ...prev, [index]: true }))}
                                                        />
                                                    </SortableItemWrapper>
                                                    <div className="w-full h-8 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><Button type="button" variant="ghost" size="sm" className="h-6 w-6 p-0 rounded-full bg-muted text-muted-foreground" onClick={() => insert(index + 1, { id: crypto.randomUUID(), TEN_SUKIEN: '', NGAY_BATDAU: '', NGAY_KETTHUC: '', MOTA: '' })}><PlusCircle className="h-4 w-4" /></Button></div>
                                                </div>
                                            ))}</div>
                                        </SortableContext>
                                    </DndContext>
