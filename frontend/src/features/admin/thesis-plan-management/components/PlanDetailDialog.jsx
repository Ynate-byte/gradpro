...
// Hiển thị thông tin chi tiết của một mục
...
export function PlanDetailDialog({ planId, isOpen, setIsOpen }) {
  ...
  // Tải chi tiết kế hoạch khi dialog mở
  React.useEffect(() => {
    if (isOpen && planId) {
      setIsLoading(true)
      setPlan(null)
  ...
  const renderStatusBadge = (status) => {
    ...
    return (
      <Badge variant="outline" className={`border-0 ${config} text-xs`}>
        {status}
      </Badge>
    )
  }

  // Định dạng ngày có thể null
  ...
  // Định dạng thời gian cho mốc thời gian
              ...
              <>
                <ScrollArea className="flex-grow overflow-y-auto">
                  <div className="p-6 space-y-6">
                    <Card>
                      ...
                      <CardContent className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-y-6 gap-x-4 text-sm">
                        ...
                        <InfoItem
                          icon={UserCircle}
                          label="Người tạo"
                          value={plan.nguoiTao?.HODEM_VA_TEN}
                        />
                      </CardContent>
                    </Card>
                    ...
                    <Card>
                      <CardHeader>
                        <CardTitle className="text-base">Các mốc thời gian</CardTitle>
                      </CardHeader>
                      <CardContent>
