        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogContent className="sm:max-w-3xl h-[80vh] flex flex-col">
                    ...
                    <ScrollArea className="h-full">
                        {isLoading ? <Loader2 className="mx-auto my-8 h-8 w-8 animate-spin" /> : (
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        <TableHead className="w-[50px]"><Checkbox checked={selected.length === students.length && students.length > 0} onCheckedChange={handleSelectAll} /></TableHead>
                                ...
                                <TableBody>
                                    {students.map(student => (
                                        <TableRow key={student.ID_NGUOIDUNG}>
                                            <TableCell><Checkbox checked={selected.includes(student.ID_NGUOIDUNG)} onCheckedChange={() => handleSelect(student.ID_NGUOIDUNG)} /></TableCell>
                ...
                <DialogFooter>
                    ...
                    <Button variant="destructive" disabled={selected.length === 0 || isProcessing} onClick={handleRemove}>
