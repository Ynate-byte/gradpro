...
import {
  searchUngroupedStudents,
  createGroupWithMembers,
} from '@/api/adminGroupService';
...
export function CreateGroupDialog({ isOpen, setIsOpen, onSuccess, planId }) {
  ...
  useEffect(() => {
    ...
    searchUngroupedStudents(planId, debouncedSearchTerm)
      ...
      .catch((error) => {
        console.error('Lỗi khi tìm sinh viên chưa có nhóm:', error);
  ...
  const availableStudents = useMemo(() => {
    ...
    return searchResults.filter(
      (student) => !selectedIds.has(student.ID_NGUOIDUNG)
    );
    ...
    try {
      ...
      setIsOpen(false);
        ...
        console.error("Lỗi khi tạo nhóm:", error);
            ...
            toast.error("Tạo nhóm thất bại", { description: errorMessage });
        } else {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      >
        <DialogHeader className="p-6 pb-4 border-b">
          ...
          <DialogDescription>
            {step === 1
              ? 'Điền thông tin cơ bản của nhóm.'
              : 'Tìm và chọn thành viên, sau đó chỉ định nhóm trưởng.'}
          </DialogDescription>
        </DialogHeader>
                    ...
                    >
                      ...
                      <div className="flex flex-col space-y-3">
                        <Label className="flex items-center gap-2">
                          <Users className="h-5 w-5" /> Sinh viên chưa có nhóm
                        </Label>
                        <div className="relative">
                          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                          <Input
                            placeholder="Lọc theo tên, MSSV (min 2 ký tự)..."
                            className="pl-10"
                            value={searchTerm}
                      ...
                      <div className="flex flex-col space-y-3">
                        <Label className="flex items-center gap-2">
                          <UserCheck className="h-5 w-5" /> Thành viên đã chọn (
                          {members.length})
                        </Label>
                            ...
                            <CardContent className="p-3 space-y-2">
                              {members.length > 0 ? (
                                  ...
                                  >
                                    <div className="flex items-center gap-3 overflow-hidden">
                                      <Avatar className="h-9 w-9 shrink-0">
                                        <AvatarFallback>
                                          {getInitials(member.HODEM_VA_TEN)}
                                        </AvatarFallback>
                                      </Avatar>
                                      <div className="flex-grow min-w-0">
                                        <p
                                          className="text-sm font-semibold truncate"
                                          title={member.HODEM_VA_TEN}
                                        >
                                          {member.HODEM_VA_TEN}
                                        </p>
                                        <p
                                          className="text-xs text-muted-foreground truncate"
                                          title={member.MA_DINHDANH}
                                        >
                                          {member.MA_DINHDANH}
                                        </p>
                                      </div>
                                    ...
                                    <div className="flex items-center gap-1 shrink-0">
                                      <Button
                                        ...
                                        onClick={() =>
                                          setLeaderId(member.ID_NGUOIDUNG)
                                        }
                                      ...
                                      <Button
                                        ...
                                        onClick={() =>
                                          handleRemoveSelected(member.ID_NGUOIDUNG)
                                        }
                                        title="Xóa khỏi nhóm"
                                      >
                                        <X className="h-4 w-4" />
                                ...
                                <div className="flex items-center justify-center h-full text-sm text-muted-foreground p-10 text-center">
                                  Chọn sinh viên từ danh sách bên trái để thêm vào
                                  nhóm.
                                </div>
                              )}
                            </CardContent>
            ...
            <DialogFooter className="p-6 border-t mt-auto shrink-0 flex justify-between sm:justify-between">
              ...
              <div>
                ...
                {step === 2 && (
                  <Button
                    type="submit"
                    disabled={isLoading || members.length === 0 || !leaderId}
                  >
                    {isLoading && (
                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    )}{' '}
                    Tạo nhóm ({members.length})
                  </Button>
                )}
              </div>
            </DialogFooter>
