...
import { Loader2, ArrowLeft, UserPlus, Search, Info } from "lucide-react";
import { Switch } from "@/components/ui/switch";
import { ScrollArea } from "@/components/ui/scroll-area";
import { cn } from "@/lib/utils"; // <-- SỬA ĐỔI: Import 'cn'

const CreateHoiDong = () => {
  const navigate = useNavigate();
  const [isManual, setIsManual] = useState(false);
  const [loading, setLoading] = useState(true); // loading trang
  const [loadingGV, setLoadingGV] = useState(false); // State loading cho GV
  ...
  // Tải Giảng viên DỰA TRÊN Kế hoạch (Giả định backend đã sửa)
  useEffect(() => {
    const fetchGiangVien = async () => {
      if (!form.ID_KEHOACH) {
        setGiangvien([]); // Xóa danh sách GV nếu bỏ chọn kế hoạch
        return;
      }

      setLoadingGV(true);
      setSelectedGV([]); // Reset lựa chọn khi đổi plan
      setGvRoles({}); // Reset vai trò

      try {
        // Gửi plan_id lên backend (backend mới sẽ trả về *tất cả* GV)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(
        isManual
          ? "Tạo hội đồng thủ công thành công!"
          : `Tạo ${payload.soLuong} hội đồng tự động thành công!`
      );
        ...
        <CardHeader>
          <div className="flex justify-between items-center mb-4">
            <div>
              ...
              <CardDescription>
                {isManual
                  ? "Tự điền thông tin và chọn giảng viên."
                  : "Tạo nhanh nhiều hội đồng cùng lúc."}
              </CardDescription>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {!isManual && (
                <div className="space-y-2">
                  <Label htmlFor="soLuong">Số lượng hội đồng</Label>
                  <Input
                    type="number"
                    id="soLuong"
                    name="soLuong"
              ...
              {isManual && (
                <div className="space-y-2">
                  <Label htmlFor="TEN_HOIDONG">Tên hội đồng (Tùy chọn)</Label>
                  <Input
                    type="text"
                    id="TEN_HOIDONG"
                    name="TEN_HOIDONG"
              ...
              <div className="space-y-2">
                <Label htmlFor="ID_KEHOACH">Kế hoạch *</Label>
                <Select
                  name="ID_KEHOACH"
                  value={form.ID_KEHOACH}
              ...
              <div className="space-y-2">
                ...
                <Select
                  name="ID_CHUYENNGANH"
                  value={form.ID_CHUYENNGANH}
                  onValueChange={(v) => handleSelectChange("ID_CHUYENNGANH", v)}
                >
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                <Label htmlFor="NGAY_BAOCAO">Ngày báo cáo (Tùy chọn)</Label>
                <Input
                  type="date"
                  id="NGAY_BAOCAO"
                  name="NGAY_BAOCAO"
              ...
              <div className="space-y-2">
                <Label htmlFor="GIO_BAOCAO">Giờ báo cáo (Tùy chọn)</Label>
                <Input
                  type="time"
                  id="GIO_BAOCAO"
                  name="GIO_BAOCAO"
              ...
              <div
                className={
                  isManual ? "md:col-span-2 space-y-2" : "space-y-2"
                }
              >
                <Label htmlFor="PHONG">Phòng báo cáo (Tùy chọn)</Label>
                <Input
                  type="text"
                  id="PHONG"
                  name="PHONG"
            ...
            {isManual && (
              <div className="mt-8 space-y-4 pt-4 border-t">
                ...
                {/* ----- SỬA ĐỔI: Bỏ max-h-64 ----- */}
                <ScrollArea className="border rounded-md p-3 bg-muted/50">
                  {loadingGV ? (
                    ...
                    <div className="flex justify-center items-center h-24 text-muted-foreground text-sm">
                      <Info className="mr-2 h-4 w-4" /> Vui lòng chọn Kế hoạch
                      để tải danh sách giảng viên.
                    </div>
                  ) : filteredGV.length > 0 ? (
                    filteredGV.map((gv) => {
                      // ----- SỬA ĐỔI: Logic kiểm tra giảng viên -----
                      const isAssigned = gv.HOIDONGS && gv.HOIDONGS.length > 0;
                      const assignedText = isAssigned
                        ? `Đã tham gia: ${gv.HOIDONGS.map(
                            (hd) => hd.TEN_HOIDONG
                          ).join(", ")}`
                        : null;
                      ...
                      return (
                        <div
                          key={gv.ID_GIANGVIEN}
                          className="flex flex-col sm:flex-row justify-between sm:items-center py-2 border-b last:border-b-0"
                          title={assignedText}
                        >
                          <Label
                          ...
                          >
                            <Checkbox
                              ...
                              onCheckedChange={() =>
                                handleToggleGV(gv.ID_GIANGVIEN)
                              }
                              disabled={isAssigned}
                            />
                          ...
                          {selectedGV.includes(gv.ID_GIANGVIEN) && (
                            <Select
                              value={
                                form.loai === "phanbien"
                                  ? "phanbien"
                                  : gvRoles[gv.ID_GIANGVIEN] || "thanhvien"
                              }
                              onValueChange={(value) =>
                                handleRoleChange(gv.ID_GIANGVIEN, value)
                              }
                              disabled={form.loai === "phanbien"}
                              className="w-full sm:w-[150px] mt-2 sm:mt-0"
                            >
            ...
            <div className="pt-6 flex justify-end">
              <Button
                type="submit"
                disabled={
                  isSubmitting || !form.ID_KEHOACH || !form.ID_CHUYENNGANH
                }
