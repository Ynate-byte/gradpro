...
class QuotaController extends Controller
{
    ...
    public function getMyQuota(Request $request)
    {
        ...
        // Đếm số đề tài đã tạo (Đã duyệt + Chờ duyệt)
        $topicsCreated = Detai::where('ID_KEHOACH', $planId)
            ->where('ID_NGUOI_DEXUAT', $lecturerId)
            ->whereIn('TRANGTHAI', ['Đã duyệt', 'Chờ duyệt'])
            ->count();

        // Đếm số nhóm thực tế đã đăng ký đề tài của GV này (để tính tải hướng dẫn)
        $actualAssigned = \App\Models\Detai::where('ID_KEHOACH', $planId)
            ->where('ID_NGUOI_DEXUAT', $lecturerId)
            ->whereIn('TRANGTHAI', ['Đã duyệt', 'Chờ duyệt'])
            ->sum('SO_NHOM_HIENTAI');

        // Tổng quota đã phân trong bộ môn
